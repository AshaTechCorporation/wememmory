...
class LoginService {
  ...
  static Future login({required String username, required String password, required String device_no, required String notify_token}) async {
    try {
      final SharedPreferences prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('token');
      final url = Uri.https(publicUrl, '/public/api/login_app');
      final headers = {'Authorization': 'Bearer $token'};
      final response = await http.post(url, body: {'username': username, 'password': password, 'device_no': device_no, 'notify_token': notify_token}).timeout(const Duration(minutes: 1));
